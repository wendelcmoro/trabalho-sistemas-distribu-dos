...
// Tarefa 4: Quando um processo correto testa outro processo correto obtém as informações do estado dos demais processos do sistema, exceto aqueles que testou nesta rodada, além do próprio testador
...
int main(int argc, char *argv[])
{
    ...
    while (time() < 150.0)
    {
        ...
        case test:
        {
            ...
            // Copia informações do processo testado
            for (int subToken = (ntoken + 1) % N; subToken != (token + 1) % N; subToken++, subToken %= N)
                if (processo[token].state[subToken] < processo[ntoken].state[subToken])
                    processo[token].state[subToken] = processo[ntoken].state[subToken];
